...
export default function MoviesPage() {
  ...
  const { movies, isLoading } = useContentLibrary();
    ...
    <ViewerLayout>
      <div className="px-5 lg:px-12 pt-8 pb-28 lg:pb-10">
        <div className="flex items-center justify-between mb-6">
          ...
          <span className="text-sm text-foreground/40">{movies.length} titles</span>
        </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-4">
            {Array.from({ length: 14 }).map((_, i) => (
              <div key={i}><Skeleton className="aspect-[2/3] rounded-2xl" /></div>
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-foreground/30">
            <p className="text-lg font-semibold">{search ? "No results found" : "No movies yet"}</p>
