...
const navItems = [
  ...
  { path: "/profile",   label: "More",     kind: "img" as const, src: "/more-icon.png" },
];
    ...
    >
      <div className="flex items-center justify-around px-2" style={{ height: 62 }}>
        {navItems.map((item) => {
          ...
          return (
            <div key={item.path} onClick={() => handleTabClick(item.path)}>
              <div className={cn(
                ...
                <div className="w-6 h-6 flex items-center justify-center">
                  {item.kind === "img" ? (
                    ...
                    <item.Icon className="w-6 h-6 transition-all duration-200" strokeWidth={1.75} />
                  )}
                </div>
                <span className={cn(
