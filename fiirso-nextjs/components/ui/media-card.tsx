...
export function MediaCard({ item, showQuality = false, onClick }: MediaCardProps) {
  ...
  return (
    <motion.div
      ...
      onClick={() => onClick ? onClick() : router.push(`/movie/${item.id}`)}
      ...
      <div className="relative w-full aspect-[2/3] rounded-2xl overflow-hidden bg-foreground/[0.06] shadow-md group-hover:shadow-[0_20px_60px_-10px_rgba(0,0,0,0.8)] transition-shadow duration-500">
        ...
        {showNew && (
          <div className="absolute top-2 left-2">
            <span className="text-[8px] font-black tracking-widest uppercase px-1.5 py-0.5 rounded-md bg-emerald-500 text-white shadow-sm">NEW</span>
          </div>
        )}
        {showHD && item.quality && (
          <div className="absolute top-2 left-2">
            <span className={cn("text-[8px] font-bold tracking-wide px-1.5 py-0.5 rounded-md backdrop-blur-sm", qualityColors[item.quality])}>
              {item.quality}
            </span>
          </div>
        )}
