...
interface SeriesFormProps {
  ...
  onSubmit: (data: SeriesFormValues) => void;
  isPending: boolean;
  isEditing: boolean;
  error?: string;
}

export function SeriesForm({ form, onSubmit, isPending, isEditing, error }: SeriesFormProps) {
  const { toast } = useToast();
  const posterUrl = form.watch("poster_url");

  const handleTmdbSelect = (data: Partial<SeriesFormValues>) => {
    Object.entries(data).forEach(([key, value]) => {
      if (value !== undefined && value !== null) form.setValue(key as keyof SeriesFormValues, value as never, { shouldDirty: true });
    });
  ...
  return (
    <div className="space-y-5 max-w-3xl mx-auto pb-10">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold tracking-tight">{isEditing ? "Edit Series" : "Add Series"}</h1>
          <p className="text-xs text-muted-foreground mt-0.5">{isEditing ? "Update series details" : "Create a new TV series"}</p>
        </div>
      </div>

      {error && <div className="p-3 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-sm">{error}</div>}
      {!isEditing && <TmdbSearch type="tv" onSelect={handleTmdbSelect} />}
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
          <Card className="rounded-2xl border-card-border">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <FormField control={form.control} name="seasons_count" render={({ field }) => (
                  <FormItem><FormLabel>Seasons</FormLabel><FormControl><Input {...field} type="number" min="0" className="rounded-xl" /></FormControl><FormMessage /></FormItem>
                )} />
              </div>
              ...
              <FormField control={form.control} name="tags" render={({ field }) => (
                <FormItem><FormLabel>Tags</FormLabel><FormControl><Input {...field} className="rounded-xl" placeholder="drama, thriller" /></FormControl>
          ...
          <div className="flex items-center gap-3 pt-1">
            ...
            <Link href="/admin/series"><Button type="button" variant="ghost" className="rounded-xl">Cancel</Button></Link>
          </div>
        </form>
      </Form>
    </div>
